...
|-----|------|---------|---------------|
| **動態** | 📢 | 庫存異動通知、系統公告 | DB07 庫存控管 |
| **交接** | 📋 | 待辦事項列表，左欄 DB03 項目、右欄 DB06 清單明細子任務 | DB03 + DB06 |
| **庫存** | 📦 | 商品庫存數量管理 | DB07 庫存控管 |
| **紀錄** | 📓 | 打卡/日誌/請假/加班（DB05 紀錄細項 + Supabase staff_activities） | DB05 登記內容 |
| **費用** | 💰 | 請款 + 請購 | DB05 登記內容（請款請購） |
...
- 對應X（9 個）= 直接上下游 / X引用（9 個）= 引用提及
- X引用：提案引用 / 管考引用 / 項目引用 / 協作引用 / 內容引用 / 明細引用 / 庫存引用 / 對象引用 / 日期引用
- Telegram Bot 寫 DB05 時：執行紀錄寫對應X、純引用寫 X引用
- Notion → Supabase 同步只取對應X，X引用 9 個 relation 純 Notion 內部使用
