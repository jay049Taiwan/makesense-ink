...
/** 動態產生 1024x1024 品牌色 placeholder（用 placehold.co，支援中文） */
function dynamicPlaceholder(text: string): string {
  const clean = text.slice(0, 16);
  return `https://placehold.co/1024x1024/${BRAND_BROWN}/${BRAND_CREAM}/png?text=${encodeURIComponent(clean)}&font=noto-sans`;
}
