...
- 角色：L5 執行長/共同創辦人
- 子品牌：旅人書店、moku 等
- [brand_naming.md](memory/brand_naming.md) — 命名鐵律：用 makesense/sms，不要用 xiansai
...
- 詳細架構見 [project_architecture.md](memory/project_architecture.md)
- [telegram_bot_setup.md](memory/telegram_bot_setup.md) — Telegram Bot 設計方向、四九 ID、待確認事項
- [hihi_concierge_design.md](memory/hihi_concierge_design.md) — 嗨嗨總管 Bot 完整設計（已決定 Plan B Mac 本地跑）
- [hihi_email_setup.md](memory/hihi_email_setup.md) — hihi@makesense.ink 主機（Namecheap）、收信轉寄、SMTP 寄信、US$14.88/年

## Notion 系統架構
- 9 個 DB 構成 XYZ 三軸座標系統（DB01~DB09 連續編號）
- DB 短名（以 Notion 實際 title 為準）：資源提案/績效管考/項目進度/協作交接/登記內容/清單明細/庫存控管/關係對象/日期紀錄
- 詳見 [notion_structure.md](memory/notion_structure.md)
- [notion_ai_agent_fields.md](memory/notion_ai_agent_fields.md) — 9 個 DB 共用的 5 個 AI 代理欄位（分析備註/檢核備註/核銷備註/ai_對應對象/ai_對應標籤；2026/05/17 退役 12 個 per-agent 欄位後精簡至此；純 Notion 內部用，跟官網/Supabase 無關）
...
- [hihi_n8n_pipeline.md](memory/hihi_n8n_pipeline.md) — v3 reviewer-in-the-loop 架構、4 個 workflow ID、每項皆「特殊工項」設計方向
- [db07_hihi_pipeline_rollout.md](memory/db07_hihi_pipeline_rollout.md) — DB07 商品掛 6 工項產簡介摘要；240 筆批量改無關卡自動跑
...
- [db_dedup_merge_capability.md](memory/db_dedup_merge_capability.md) — DB01-09 重複偵測與合併＝總管常駐能力，兩層架構，DB08 清理是第一次實跑
...
- **協作回覆**（text，2026/05/30 從「交接回覆」改名）
- 執行時間（date）：有 end date 時計算 duration_min（分鐘差值），無 end date 預設 120 分鐘
- **距離km**（number，**無括號**）→ events.distance_km → point_ledger type=距離行程
- **對應內容**（relation → DB05）

### 2026/05/30 Schema 大變動（重整）

**Database 改名**：
- 舊「DB04 協作交接」→ 新「DB04 交接協作」
- 舊「DB09 日期紀錄」→ 新「DB09 時程日誌」

**Property rename**（9 DB 跨表）：
- 「日期引用」→「日誌引用」（9 DB）
- 「日期被引」→「日誌被引」（9 DB）
- 「交接回覆」→「協作回覆」（9 DB）
- DB03：「交接已完成標記」→「協作已完成標記」、「交接未完成標記」→「協作未完成標記」
- DB04：「交接摘要」→「協作摘要」（**「交接類型」select 保留不動**：庫存門市仍歸交接邏輯）
- DB05：「交接備註」→「協作備註」、「交接日期」→「協作日期」
- DB06：「交接日期」→「協作日期」

**新增欄位**（9 DB 各 N 個）：
- 9 個「聯想 X」relation（聯想內容／協作／對象／庫存／提案／日誌／明細／管考／項目）— 「跨域聯想」用、嗨嗨聯想唯一可寫
- 1 個「企劃備註」text — 嗨嗨企劃唯一可寫；target 在 DB01-05 寫章節架構、DB06-09 寫資料筆記

**權限獨佔調整**：
- 「對應標籤」relation：改由嗨嗨檢核獨佔（原家族都可寫；嗨嗨搜查／嗨嗨分析仍可新建 DB08 page，但不連入 target）
- 「執行構想」：改由嗨嗨企劃唯一寫入（總管不再 append、改為對話產出文字回給四九複製貼上）
- 「企劃備註」：唯一由嗨嗨企劃寫入

**對應變動文檔**：
- 4-1-1 嗨嗨家族導航地圖（能力矩陣已更新）
- 4-2-1-1 ~ 4-2-7-1 嗨嗨原廠設定（七頁 title 從「工作指示」改「原廠設定」、內容對齊新規則）
- 2-2-1 欄位名稱指南（既有欄位描述同步、加 9 個聯想 row）
- 3-4-3 數位內容執行指南（新建）
...
- 完整說明見 [notion_structure.md](memory/notion_structure.md)「對應 / 引用 / 被引」段

## 系統一致性稽核（2026-04-28 ~ 05-05 多輪）
**最終 latest 報告（已超越舊版）：**
- [audit_pairing_failures_2026-04-29_v3.md](memory/audit_pairing_failures_2026-04-29_v3.md) — 程式↔Supabase↔Notion 配對（v3 最終）
- [audit_docs_alignment_2026-04-29_v3.md](memory/audit_docs_alignment_2026-04-29_v3.md) — 文件對齊（v3，含 custom skill）
- [audit_db_schema_2026-04-28.md](memory/audit_db_schema_2026-04-28.md) — DB schema 深度
- [audit_ai_agent_fields_2026-04-28.md](memory/audit_ai_agent_fields_2026-04-28.md) — 14 AI 欄位

**指南頁稽核（2026-04-29）：**
- [audit_notion_guides_p1_2026-04-29.md](memory/audit_notion_guides_p1_2026-04-29.md) — 系統總論+資料庫類別
- [audit_notion_guides_p2_2026-04-29.md](memory/audit_notion_guides_p2_2026-04-29.md) — 文案+圖像+組織
- [audit_notion_guides_p3_2026-04-29.md](memory/audit_notion_guides_p3_2026-04-29.md) — 各類總頁+官網2份
...
- [makesense_ink_tech.md](memory/makesense_ink_tech.md) — 技術棧、頁面結構、Supabase 表、三種入口/角色、開發狀態
- [makesense_ink_vision.md](memory/makesense_ink_vision.md) — 一站三角色、LINE 角色互動、圖書館級內容量、商業化願景
- [makesense_ink_supabase_migration_done.md](memory/makesense_ink_supabase_migration_done.md) — 2026/04/13 全站 Supabase 遷移完成 + 即時同步機制
- [ticket_product_model.md](memory/ticket_product_model.md) — 票券放 DB07 作通用商品，跨活動共用
- **官網同步區**：Notion 頁面 `3419ff25fdab80f59b03fcafbd9c7bb8`，每個 DB view 對應官網一個元素
- [line_integration_handoff.md](memory/line_integration_handoff.md) — LINE 整合交接文件（待驗證項目清單）
- [page_structure_footer_pages.md](memory/page_structure_footer_pages.md) — Footer 四頁面上下半部架構（DB05 官網備項 + Supabase 動態資料）
...
- [feedback_no_rest_suggestion.md](memory/feedback_no_rest_suggestion.md) — 不要叫用戶去休息，他會自己決定什麼時候停
- [feedback_be_accurate.md](memory/feedback_be_accurate.md) — 講話要確實，不確定就說不確定，不要說完成了結果壞掉
- [feedback_db06_mingxi_type.md](memory/feedback_db06_mingxi_type.md) — 訂單寫入 DB06 時「明細類型」一律「報名登記」
- [feedback_frame_work_for_user.md](memory/feedback_frame_work_for_user.md) — 描述工作量時用對使用者的影響來框架，技術難度是我的事
...
- [feedback_difang_tongxun_wordcount.md](memory/feedback_difang_tongxun_wordcount.md) — 地方通訊月刊 600-900/季刊 900-1500/年刊 1000-4000 字；禁氛圍詞，要具名事件+日期+人物+數字
- [feedback_no_punt_to_next_baton.md](memory/feedback_no_punt_to_next_baton.md) — 跑某項工作時意外發現的本職資料要當場補，不可推給下一項
- [feedback_no_simplified_chinese.md](memory/feedback_no_simplified_chinese.md) — 不喜歡簡體中文，採集入 Notion 一律 opencc s2tw 轉繁體
...
- [feedback_notion_full_name_link.md](memory/feedback_notion_full_name_link.md) — 提 Notion 頁面**永遠**寫「完整名稱 + 超連結」,不寫純編號(如 4-2-4-1)也不寫純名字。四九已多次要求、會煩。
...
- [dt_2015_pipeline_status.md](memory/dt_2015_pipeline_status.md) — 17 頁管考工項第二輪完成（管考備註/分析備註/內容名稱/執行備註），DB08 對應 + 後 4 項待下一 session
...
- [onedrive_photo_migration.md](memory/onedrive_photo_migration.md) — 17萬張 OneDrive 照片大遷移計畫

## 【待開發】政府招標決標每日監測
- [pcc_tender_monitor_plan.md](memory/pcc_tender_monitor_plan.md) — 每日 6:30 掃政府電子採購網招標/決標、比對 DB08 既有單位；P0 卡關＝DB08「單位選項」待四九 backfill
