...
## 【待辦】舊批次嗨嗨分析內容區塊 CJK 損字清理（2026/05/22 四九：要，之後安排）
- 範圍：DB05 文案類（提案申請／成果報告等）2026-04-17/18 第一批分析寫的「🤖 嗨嗨分析」content block，早於後來的 CJK 防損字（re-fetch 驗證）紀律，內文有殘留亂碼/形近錯字。
- 已知實例：112實體書店補助申請書（`2809ff25fdab811aa88fd23c94026a40`）內容區塊「半年營收**出均**」（應為平均）、「餐飲**處所指屵**4點3款」（屵 為亂碼）。其餘舊批次頁可能同樣有問題。
- 本次（2026/05/22）補建只補分析備註 + DB06 記錄，**未動內容區塊**，故損字仍在。
- 待辦動作：盤點所有 2026-04 第一批分析的 content block → 逐頁 re-fetch 比對 → 用 replace_content 改正損字 → 改完再 re-fetch 驗證。
