...
### 結構性憑證（鐵律）
每分析一個 page，憑證落在 **DB06 baton 的兩個結構性條件**，不只靠 target 的 `分析備註` token：
1. **掛 relation**：來源 page 的 `對應明細` 必須掛一筆 DB06 baton，欄位組合 = 分析（`ai模式=分析`）。
2. **執行狀態=完成**：收尾時把那筆 DB06 baton 的 **`執行狀態` 設為「完成」**（注意：這是 `執行狀態` 欄，跟既有的 `ai狀態=完成` 是兩個不同欄位；兩個都要設）。
> 用意：`對應明細` relation + `執行狀態=完成` 是可查詢、可跑報表的結構性憑證，比純文字 `分析備註` token 更可靠，能直接掃出「哪些 page 已被分析過 / 還沒」。

### 掃描順序（逐類掃描）
DB05 分類別逐一掃完，順序：**成果報告 → 提案申請 → 其餘類別逐一**。每個類別用 dbanly 掃出對應標籤為空的 page，一批批清。

### 補建範圍
**此規則由 2026-05-22 起往後套用**；本 session（及更早）已建、未掛 `對應明細`／`執行狀態` 還是執行中的舊分析棒 **不回頭補建**。
