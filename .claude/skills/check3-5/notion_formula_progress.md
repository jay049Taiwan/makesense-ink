...
⬅️ **下一個 Session 從 DB07 Phase 2 開始**（0f5a87d4-d1df-4271-ba00-2abfee01693d）。

**DB07 要加：社群×5 + 財務C×4 = 9 條**
- relation to DB06：grep schema 確認（預期叫 `對應明細`）
- 財務 Batch 1：🆕售出總計 ROLLUP('對應明細', '售出小計', 'sum') + 🆕進貨總計 ROLLUP('對應明細', '進貨小計', 'sum')
- 財務 Batch 2（rename 後加）：🆕商品毛利 FORMULA('prop("售出總計") - prop("進貨總計")') + 🆕商品毛利率（內嵌展開，不走 prop("商品毛利")）

**DB08 要加：社群×5 = 5 條**（無財務、無時間進度）
- relation to DB06：grep schema 確認
