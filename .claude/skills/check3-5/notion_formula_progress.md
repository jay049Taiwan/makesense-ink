...
|----|---------------|---------|---------|
| **DB01 資源提案** | 722f2478-7e61-4b4b-ad1c-d171b4a639db | ✅ 5條+rename | ✅ Phase 2 完成（6條：社群×5 + 時間進度）|
| **DB02 績效管考** | c286e19b-9cf8-422b-8628-98b6d116040c | ✅ 5條+rename | ✅ Phase 2 完成（17條）|
| **DB03 項目進度** | 968b23ea-da1f-4381-bd9a-253ee80b0656 | ✅ 5條+rename | ✅ Phase 2 完成（14條）|
| **DB04 協作交接** | 5ad63416-a7c5-4d84-812e-cddf56c8bc01 | ✅ 5條+rename | ✅ Phase 2 完成（14條）|
| **DB05 登記內容** | 28a667a9-ede1-466a-9f18-419da33a8810 | ✅ 5條+rename | ✅ Phase 2 完成（10條，⚠️ 跳過 預算總計）|
| **DB06 清單明細** | a809ff25-fdab-8236-b491-87496d236ac9 | ✅ 5條+rename | — (DB06 是 rollup 的資料源，不加) |
| **DB07 庫存控管** | 0f5a87d4-d1df-4271-ba00-2abfee01693d | ✅ 5條+rename | ✅ Phase 2 完成（9條：社群×5 + 售出/進貨/商品毛利/商品毛利率）|
| **DB08 關係對象** | 6934a808-b79b-4446-98dd-f699476408a0 | ✅ 4條+rename | ✅ Phase 2 完成（5條：社群×5）|
...
ADD COLUMN "🆕結算毛利率" FORMULA('if(prop("預算總計") > 0, format(round((prop("預算總計") - prop("支出總計")) / prop("預算總計") * 100)) + "%", "—")')
```
- **結算毛利率**：⚠️ 不能用 `prop("結算毛利")`（formula→formula type error）→ 展開為 `(預算總計 - 支出總計) / 預算總計`
- `== 0` 對 rollup 比較型 type error → 改 `> 0`（條件反轉，先 true branch 輸出 %）
...
ADD COLUMN "🆕預計毛利率" FORMULA('if(prop("提案營業額") > 0, format(round((prop("提案營業額") - prop("預算總計")) / prop("提案營業額") * 100)) + "%", "—")')
```
...
| `prop("formula欄")` 用在新 formula，且該 formula欄 本身引用 formula-type rollup | ❌ type error | 直接從 rollup 欄計算，不經中間 formula |
| `prop("rollup欄") == 0` | ❌ type error（rollup 型比較）| 改用 `prop("rollup欄") > 0` |
| `prop("formulaRollupA") > 0 \|\| prop("formulaRollupB") > 0`（formula-type rollup）| ❌ type error | 改用巢狀 `if`：`if(prop("A") > 0, ..., if(prop("B") > 0, ...))` |
| `empty(prop("dateA")) \|\| empty(prop("dateB"))` 在時間進度 | ❌ type error | 改用雙層 `if(empty(A), "—", if(empty(B), "—", ...))` |
| `ROLLUP('rel', '預算小計', 'sum')` 若 DB06 的 預算小計 是 rollup type | ❌ Notion 不允許 rollup→rollup | 跳過，或用 DB06 base number 欄位（登記售價×數量）替代 |
...
5. **Formula-type rollup → formula 鏈**：若 rollup 的 targetPropertyType=formula（即 rollup 目標是 DB06 的 formula 欄），其值可能有 type error propagation。在此 rollup 上加減乘除得到的中間 formula 欄，**無法被另一個 formula 引用**（prop() 直接 type error）。解法：毛利率等衍生公式**內嵌展開**，不走 prop("中間毛利欄")，改寫成 prop("收入rollup") - prop("支出rollup") 直接計算。
...
~~DB02 Phase 2~~ ✅ 完成（Session E）

~~DB03 Phase 2~~ ✅ 完成（Session F）
- 8 rollups（按讚/留言/分享/觸及/觀看/收入/支出/預算 from `對應明細`→DB06）
- 1 時間進度 formula
- 4 財務 formula（金額狀態/總和總計/結算毛利/結算毛利率）
- **DB03 特例**：結算毛利率用`收入總計`-based（不用`預算總計`-based），因 DB03 有實際收入資料
- **新坑**：formula-type rollup 條件用 `||` 時 type error → 改巢狀 `if()`（已記入 syntax table）

~~DB04 Phase 2~~ ✅ 完成（Session F）
- DB04 已有 6 rollups（按讚/留言/分享/觀看/支出/預算），新增 觸及總計 + 收入總計
- 日期欄位確認：`起算日期` + `截止時間`（與 DB02/DB03 相同名）
- `||` 在時間進度也 type error → 改巢狀 if（兩層 empty check）
- 同 DB03 財務公式（收入總計-based）

~~DB05 Phase 2~~ ✅ 完成（Session F）
- DB05 原有 按讚/留言/分享/觸及/觀看/預算 均為 formula type → 先 rename 舊_*，再加 rollup 版本
- `預算小計` 在 DB06 為 rollup type → rollup-of-rollup 不合法，跳過 `預算總計` rollup
- 財務公式改為純 收入/支出 2 欄版（不含 預算 branch）
- ⚠️ 若後續需要 DB05 的 `預算總計`，Noah 可手動加 number 欄位計算

~~DB01 Phase 2~~ ✅ 完成（Session G）
- 日期欄位：起算日期 + 截止時間（已確認存在於 DB01 schema）
- relation to DB06：`對應明細`
- 6 條：按讚/留言/分享/觸及/觀看（rollup）+ 時間進度（formula，雙層 empty 巢狀 if）

~~DB07 Phase 2~~ ✅ 完成（Session G）
- 7 rollups（社群×5 + 售出總計/進貨總計 from `對應明細`→DB06）
- 2 formulas（商品毛利/商品毛利率，毛利率用內嵌展開不走中間欄位）

~~DB08 Phase 2~~ ✅ 完成（Session G）
- 5 rollups（按讚/留言/分享/觸及/觀看 from `對應明細`→DB06）

🎉 **Phase 2 全部 DB 完成！所有 Notion formula 改造 Phase 1 + Phase 2 完畢。**
...
Session F(2026-05-29 Session F):
- DB03 項目進度 Phase 2 全部完成（14 條欄位）
  - 8 rollups（社群×5 + 收入/支出/預算 from 對應明細→DB06）
  - 1 時間進度 formula
  - 4 財務 formula（金額狀態/總和總計/結算毛利/結算毛利率）
- DB03 特例：結算毛利率用收入總計-based（不用預算總計-based）
- 新坑：formula-type rollup 在 || 條件中 type error → 改巢狀 if()
- DB04 協作交接 Phase 2 全部完成（14 條欄位）
  - 已有 6 rollups，新增 觸及總計 + 收入總計（共 8 rollups）
  - 1 時間進度 + 4 財務 formula（金額狀態/總和總計/結算毛利/結算毛利率）
  - 額外坑：時間進度的 `||` 也 type error，改雙層巢狀 if(empty)
- DB05 登記內容 Phase 2 完成（10 條，跳過 預算總計）
  - 原有 按讚/留言/分享/觸及/觀看/預算 都是 formula type → rename 舊_* → 加 rollup
  - 預算小計 在 DB06 是 rollup → rollup-of-rollup 被 Notion 拒絕，DB05 跳過 預算總計
  - 財務公式改為 收入/支出 2 欄版（不含 預算 branch）
- 本次 session 沒改 repo 程式碼，只動 Notion live + 更新本 progress 文件

Session G(2026-05-29 Session G):
- DB01 資源提案 Phase 1 cleanup：4 個 🆕 前綴欄位 rename 完成（前 session 遺留）
  - 被引總數衝突：先 rename 舊→舊_被引總數，🆕被引總數→被引總數（兩次 DDL 繞 race condition）
- DB01 Phase 2 全部完成（6 條）
  - 5 rollups（按讚/留言/分享/觸及/觀看 from `對應明細`→DB06）
  - 1 時間進度 formula（雙層巢狀 empty check，`起算日期` + `截止時間`）
- DB07 庫存控管 Phase 2 全部完成（9 條）
  - 5 社群 rollups + 售出總計/進貨總計 rollups（all from `對應明細`→DB06）
  - 商品毛利 formula + 商品毛利率 formula（毛利率內嵌展開，`if(售出總計>0, ...)`）
- DB08 關係對象 Phase 2 全部完成（5 條）
  - 5 社群 rollups from `對應明細`→DB06
- 🎉 Phase 2 全部完成！所有 Notion formula 改造 Phase 1 + Phase 2 全部完畢（9 DB）
- 本次 session 沒改 repo 程式碼，只動 Notion live + 更新本 progress 文件
