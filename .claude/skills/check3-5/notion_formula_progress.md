...
⬅️ **下一個 Session 從 DB03 Phase 2 開始**（968b23ea-da1f-4381-bd9a-253ee80b0656）。

**DB03 要加：時間進度 + 金額狀態 + 社群×5 + 財務A×6 = 14 條**
- 參照 DB02 流程
- 率公式（結算毛利率）內嵌展開：`if(prop("預算總計") > 0, format(round((prop("預算總計") - prop("支出總計")) / prop("預算總計") * 100)) + "%", "—")`（不 prop("結算毛利")）
