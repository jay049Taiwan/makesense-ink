...
- **DB03 特例**：結算毛利率用`收入總計`-based（不用`預算總計`-based），因 DB03 有實際收入資料
  - `金額狀態`: `if(prop("預算總計") > 0, if(prop("支出總計") > 0, "有支出", "有預算"), if(prop("收入總計") > 0, "有收入", "—"))`
  - `總和總計`: `if(prop("預算總計") > 0, "預:" + format(prop("預算總計")) + " 支:" + format(prop("支出總計")), if(prop("收入總計") > 0, "收:" + format(prop("收入總計")), "—"))`
  - `結算毛利`: `prop("收入總計") - prop("支出總計")`
  - `結算毛利率`: `if(prop("收入總計") > 0, format(round((prop("收入總計") - prop("支出總計")) / prop("收入總計") * 100)) + "%", "—")`
- **新坑**：formula-type rollup 條件用 `||` 時 type error → 改巢狀 `if()`（已記入 syntax table）

⬅️ **下一個 Session 從 DB04 Phase 2 開始**（5ad63416-a7c5-4d84-812e-cddf56c8bc01）。

**DB04 要加：時間進度 + 金額狀態 + 社群×5 + 財務A×6 = 14 條**
- 參照 DB03 流程（DB04 與 DB03 結構相同）
- 先 fetch schema 確認：`起算日期`/`截止時間` 欄位名 + `對應明細` relation 名
