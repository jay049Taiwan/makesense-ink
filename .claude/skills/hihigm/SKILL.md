...
② 沒範例 → 派子 agent 查 Notion:跟 target 完全匹配欄位組合的其他 page
   ...
   多筆時取 top 3 給四九挑,排序規則(「黃金範例優先」落地版):
   ① 主排序 = 被引總數 由高到低(8 個 target DB 都有此 formula 欄位,
      值 = 9 個「被引」relation 筆數加總;高 = 被家族一再回頭參考 = 經實證的好範本)
   ② 通過審查者優先:該 DB 若有「檢核狀態」欄位,只取其「完成/綠燈」群組值的 page
      (各 DB 綠燈值不一 — DB03/DB05=確認ok、DB02=已檢核、DB01=已封存、DB09=完成,
       Agent A 讀該 DB schema 動態判定,不寫死);無「檢核狀態」欄位的 DB(DB04/07/08)
      退用「發佈狀態=已發佈」當通過代理,再無則略過此門檻
   ③ tiebreak = 最近完成日期
   「黃金範例」= 通過審查 + 被引總數高,不靠主觀點名,由家族引用行為自然浮現。
...
3. 否則 → 查 target 所在 DB 內同欄位組合的其他 page(完全匹配):
   - 用 Notion MCP query/fetch tool(先 ToolSearch
     "select:mcp__73449d04-01f2-4d79-b0b5-908d1d64b42d__fetch")
   - 撈候選後依「黃金範例」排序取 top 3:
     ① 主排序 = 被引總數 由高到低(該 formula 欄位 8 個 target DB 都有)
     ② 通過審查優先:先讀該 DB「檢核狀態」schema 的「完成/綠燈」群組值,
        只留該值的 page(各 DB 綠燈值不同,動態判定不寫死);
        該 DB 無「檢核狀態」欄位 → 改用「發佈狀態=已發佈」,再無則略過
     ③ tiebreak = 最近完成日期
4. 若完全匹配 0 筆 → 回傳「沒有對標」

回傳格式:
- 模式 = ① / ② / ③
- 候選清單(每筆:title、URL、被引總數值、執行構想摘要 1-2 句)
