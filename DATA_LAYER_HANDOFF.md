...
| 內容類型 | select | `報名登記`（DB05 只有 3 選項：報名登記/共識互動/內容素材） |
| 對應對象 | relation → DB08 | 廠商的 DB08 page ID（如有） |

> **2026/05/06 補**：DB05 新增 9 個 X引用 relation（提案引用/管考引用/項目引用/協作引用/內容引用/明細引用/庫存引用/對象引用/日期引用），語意「此 page 主動引用他人」（出向）。
> **2026/05/08 補**：DB05 再新增 9 個 X被引 relation（提案被引/管考被引/項目被引/協作被引/內容被引/明細被引/庫存被引/對象被引/日期被引），語意「此 page 被他人引用」（入向）。**X被引 是 X引用 的 dual-sync 反向，由 Notion 自動鏡射，AI 不需操作；人工裁切只需在 X引用 端進行**（已驗證 9 對全部都是 dual-sync）。
> X引用 + X被引 共 18 個 relation **不參與 Supabase 同步**。聚合查詢只看「對應X」系列。
