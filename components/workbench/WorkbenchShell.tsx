    ...
    <div className="grid sm:grid-cols-[320px_1fr] gap-0" style={{ minHeight: 500 }}>
      <div style={{ borderRight: "1px solid #f0f0f0", overflowY: "auto", maxHeight: "calc(100vh - 200px)" }}>
        ...
        <div className="px-3 py-2">
          <p className="text-[10px] font-bold px-2 py-1 rounded" style={{ background: "#7a5c40", color: "#fff", display: "inline-block" }}>未完成 {unfinished.length}</p>
        </div>
        {unfinished.map((t) => (
          <button key={t.id} onClick={() => setSelectedId(t.id)} className="w-full text-left px-4 py-3 transition-colors"
            style={{ borderBottom: "1px dashed #e8e0d4", background: selectedId === t.id ? "#f5f0e8" : "transparent", borderLeft: selectedId === t.id ? "3px solid #7a5c40" : "3px solid transparent", cursor: "pointer", border: "none", borderBottomStyle: "dashed" as any, borderBottomWidth: 1, borderBottomColor: "#e8e0d4" }}>
            <p className="text-sm font-semibold" style={{ color: "#333" }}>{t.title}</p>
            <p className="text-xs mt-0.5" style={{ color: "#999" }}>{t.person} ・ {t.subtasks.filter(s => s.done).length}/{t.subtasks.length} 完成</p>
          </button>
        ))}
        {finished.length > 0 && (
          <>
            <div className="px-3 py-2 mt-2">
              <p className="text-[10px] font-bold px-2 py-1 rounded" style={{ background: "#ccc", color: "#fff", display: "inline-block" }}>已完成 {finished.length}</p>
            </div>
            {finished.map((t) => (
              <button key={t.id} onClick={() => setSelectedId(t.id)} className="w-full text-left px-4 py-3"
                style={{ borderBottom: "1px dashed #e8e0d4", background: selectedId === t.id ? "#f5f0e8" : "transparent", cursor: "pointer", border: "none", borderBottomStyle: "dashed" as any, borderBottomWidth: 1, borderBottomColor: "#e8e0d4" }}>
          ...
          <div className="h-full flex flex-col">
            <div className="p-4" style={{ borderBottom: "1px solid #e8e0d4", background: selected.done ? "#f9f9f9" : "#fff" }}>
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-base font-bold" style={{ color: selected.done ? "#aaa" : "#333", textDecoration: selected.done ? "line-through" : "none" }}>{selected.title}</h4>
              ...
              {selected.note && <p className="text-xs mt-2 px-3 py-2 rounded" style={{ background: "#faf8f4", color: "#666" }}>📝 {selected.note}</p>}
            </div>
            <div className="flex-1 p-4" style={{ background: "#fafafa" }}>
              <p className="text-xs font-semibold mb-3" style={{ color: "#888" }}>相關進銷明細（DB06）— {selected.subtasks.filter(s => s.done).length}/{selected.subtasks.length} 已完成</p>
              <div className="space-y-2">
                {selected.subtasks.map((sub) => (
                  <label key={sub.id} className="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors" style={{ background: "#fff", border: sub.done ? "1px solid #d4edda" : "1px solid #e8e0d4" }}>
                    <input type="checkbox" checked={sub.done} onChange={() => toggleSubtask(selected.id, sub.id)} className="w-4 h-4 accent-green-600 flex-shrink-0" />
                    <span className="text-sm" style={{ color: sub.done ? "#aaa" : "#333", textDecoration: sub.done ? "line-through" : "none" }}>{sub.title}</span>
                    {sub.done && <span className="ml-auto text-xs" style={{ color: "#4CAF50" }}>✓</span>}
                  </label>
                ))}
              </div>
              {selected.subtasks.every(s => s.done) && (
                <div className="mt-4 p-3 rounded-lg text-center text-sm" style={{ background: "#d4edda", color: "#155724" }}>✅ 所有項目已完成，此工作已結案</div>
              )}
            </div>
          </div>
        )}
    ...
    <div className="grid sm:grid-cols-[240px_1fr] gap-0" style={{ minHeight: 500 }}>
      <div className="p-4" style={{ borderRight: "1px solid #f0f0f0" }}>
        ...
        {(["請款", "請購"] as const).map((m) => (
          <button key={m} onClick={() => setMode(m)} className="w-full text-left px-4 py-3 rounded-lg mb-2 text-sm transition-colors"
            style={{ background: mode === m ? "#7a5c40" : "transparent", color: mode === m ? "#fff" : "#666", border: mode === m ? "none" : "1px solid #e8e0d4", fontWeight: mode === m ? 700 : 400, cursor: "pointer" }}>
            {m === "請款" ? "💸 請款（報銷）" : "🛒 請購（採買）"}
          </button>
        ))}
        <div className="mt-6 p-3 rounded-lg" style={{ background: "#faf8f4" }}>
          <p className="text-xs" style={{ color: "#888" }}>本次{mode}金額</p>
          <p className="text-xl font-bold mt-1" style={{ color: "#7a5c40" }}>NT$ {(mode === "請款" ? claimTotal : purchaseTotal).toLocaleString()}</p>
        </div>
      </div>
          ...
          <>
            ...
            {claimItems.map((item, i) => (
              <div key={i} className="grid grid-cols-[1fr_100px_80px_auto] gap-2 mb-3 items-end">
                <div>{i === 0 && <label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>請款項目</label>}
                  <input type="text" value={item.name} onChange={(e: any) => updateClaim(i, "name", e.target.value)} placeholder="例：走讀講師費" className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none" }} /></div>
                <div>{i === 0 && <label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>單價</label>}
                  <input type="number" value={item.price || ""} onChange={(e: any) => updateClaim(i, "price", Number(e.target.value))} placeholder="0" className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none" }} /></div>
                <div>{i === 0 && <label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>數量</label>}
                  <input type="number" value={item.qty} onChange={(e: any) => updateClaim(i, "qty", Number(e.target.value))} min={1} className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none" }} /></div>
                <div className="flex items-center"><span className="text-sm font-bold" style={{ color: "#7a5c40", whiteSpace: "nowrap" }}>= {(item.price * item.qty).toLocaleString()}</span></div>
              </div>
            ))}
            ...
            <div className="flex justify-end mb-6 px-2"><div className="text-right"><span className="text-xs" style={{ color: "#888" }}>合計金額</span><p className="text-2xl font-bold" style={{ color: "#7a5c40" }}>NT$ {claimTotal.toLocaleString()}</p></div></div>
            <div className="mb-4"><label className="text-xs font-semibold block mb-2" style={{ color: "#888" }}>核銷憑證</label>
              <div className="flex gap-3">{(["有", "無"] as const).map((v) => (<label key={v} className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: "#333" }}><input type="radio" name="receipt" checked={claimReceipt === v} onChange={() => setClaimReceipt(v)} className="accent-amber-700" />{v === "有" ? "✅ 有憑證" : "❌ 無憑證"}</label>))}</div></div>
            <div className="mb-4"><label className="text-xs font-semibold block mb-1" style={{ color: "#888" }}>備註</label>
              <textarea value={claimNote} onChange={(e: any) => setClaimNote(e.target.value)} placeholder="請補充說明..." rows={3} className="w-full px-3 py-2 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", resize: "vertical" }} /></div>
            <div className="mb-6"><label className="text-xs font-semibold block mb-1" style={{ color: "#888" }}>上傳照片（收據/發票）</label>
              <div className="flex items-center justify-center h-24 rounded-lg cursor-pointer hover:opacity-80" style={{ border: "2px dashed #ddd", background: "#fafafa" }}><span className="text-sm" style={{ color: "#aaa" }}>📷 點擊上傳或拖曳檔案</span></div></div>
            <button className="w-full py-3 rounded-lg text-sm font-bold text-white" style={{ background: "#7a5c40", border: "none", cursor: "pointer" }}>送出請款申請</button>
          </>
        ) : (
          <>
            ...
            {purchaseItems.map((item, i) => (
              <div key={i} className="p-4 rounded-lg mb-3" style={{ border: "1px solid #e8e0d4", background: "#fafafa" }}>
                <div className="grid grid-cols-[1fr_100px_80px] gap-2 mb-2">
                  <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>品名</label><input type="text" value={item.name} onChange={(e: any) => updatePurchase(i, "name", e.target.value)} placeholder="例：A4影印紙" className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", background: "#fff" }} /></div>
                  <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>單價</label><input type="number" value={item.price || ""} onChange={(e: any) => updatePurchase(i, "price", Number(e.target.value))} placeholder="0" className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", background: "#fff" }} /></div>
                  <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>數量</label><input type="number" value={item.qty} onChange={(e: any) => updatePurchase(i, "qty", Number(e.target.value))} min={1} className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", background: "#fff" }} /></div>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>備註</label><input type="text" value={item.note} onChange={(e: any) => updatePurchase(i, "note", e.target.value)} placeholder="選填" className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", background: "#fff" }} /></div>
                  <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>參考連結</label><input type="url" value={item.url} onChange={(e: any) => updatePurchase(i, "url", e.target.value)} placeholder="https://..." className="w-full h-9 px-3 rounded-lg text-sm" style={{ border: "1px solid #ddd", outline: "none", background: "#fff" }} /></div>
                </div>
                <div><label className="text-[10px] font-semibold block mb-1" style={{ color: "#888" }}>商品照片</label>
                  <div className="flex items-center justify-center h-16 rounded-lg cursor-pointer" style={{ border: "1px dashed #ddd", background: "#fff" }}><span className="text-xs" style={{ color: "#aaa" }}>📷 上傳照片</span></div></div>
                <div className="text-right mt-2"><span className="text-sm font-bold" style={{ color: "#7a5c40" }}>小計 NT$ {(item.price * item.qty).toLocaleString()}</span></div>
              </div>
            ))}
            ...
            <div className="flex justify-end mb-6"><div className="text-right"><span className="text-xs" style={{ color: "#888" }}>合計金額</span><p className="text-2xl font-bold" style={{ color: "#7a5c40" }}>NT$ {purchaseTotal.toLocaleString()}</p></div></div>
            <button className="w-full py-3 rounded-lg text-sm font-bold text-white" style={{ background: "#7a5c40", border: "none", cursor: "pointer" }}>送出請購申請</button>
          </>
        )}
      </div>
    </div>
