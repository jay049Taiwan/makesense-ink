...
> ⚠️ **此檔為 2026/04/20 snapshot，已過時**：上線前未解決項目部分已完工（A. checkout Notion 回寫 + 扣庫存 → DB06 直接更新 stock；C. 資料量已大量發佈）。**現況以 makesense-ink/CLAUDE.md 為準**。
...
| 9 | 首頁 `<title>` 顯示「現思文化創藝 | makesense」太生硬 | 改為「旅人書店/宜蘭文化俱樂部」 |
...
**原因**：專案位於 `~/Code/makesense-ink/`，Mac 預設把 Documents 同步到 iCloud Drive。本地刪除後，iCloud 從雲端還原舊版檔案。
...
### 本機路徑
- 專案：`/Users/jay049/Code/makesense-ink/`（⚠️ 在 iCloud Drive 同步中）
- Python photo_processor：`/Users/jay049/Documents/工作參考資料/photo_processor/`（已於 2026/05/09 刪除）
