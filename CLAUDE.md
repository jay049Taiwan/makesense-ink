...
### DB08 同步篩選規則
DB08 分類用兩個欄位：`標籤狀態`（status：**觀點 / 追蹤**）與 `對象類型`（select：個人/組織/地點/事件/**類別**/導覽帶路）。
（舊 `經營類型` select 已移除，2026/05 遷移到 `標籤狀態`；勿再用經營類型）

| Supabase 表 | DB08 過濾條件 |
|-------------|--------------|
| topics | 標籤狀態=觀點→tag_type='viewpoint'；標籤狀態=追蹤→'tag'；對象類型=類別→'category'（商品分類，會員商品掛「對應類別商品」relation） |
| persons | 會員狀態=會員 AND 關係選項=個人 |
| partners | 會員狀態=會員 AND 關係選項=合作夥伴 |
| staff | 會員狀態=會員 AND 關係選項=工作團隊 |
| members | 會員狀態=會員（不論關係選項，以 email 為主鍵） |

**同一筆 DB08 可同時寫入多表**（例：帶路老師「標籤狀態=觀點 + 關係選項=個人」會同時出現在 topics 和 persons）
