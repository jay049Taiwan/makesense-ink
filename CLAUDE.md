...
還在用 mock data 需要改的頁面：
- ❌ /market-booking — 自有產品、合作品牌、市集活動
- ❌ /content-curation — 統計數字、採輯主題
- ❌ /reading-tour — 走讀關鍵字、去過的地點
- ❌ /events/[slug] — 活動詳情
- ❌ /goods-selection — 風格選物專頁
- ❌ /dashboard/partner — 合作後台
- ❌ /dashboard/products — 商品管理
- ❌ /checkout — demo items（購物車空時注入的假資料）

統一的 Supabase 查詢函式庫：lib/fetch-supabase.ts
- fetchSBProducts(subCategory?, limit) — 商品
