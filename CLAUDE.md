...
仍用 mock data 的頁面（刻意保留）：
- /dashboard — 會員購買紀錄（MOCK_MEMBER_PURCHASES）待接 Supabase orders
- /buy/[slug] — 市集預購頁（MOCK_ACTIVITIES, MOCK_PRODUCTS）待接 Supabase
- /checkout — demo items（購物車空時注入的假資料，開發測試用）
