...
### 上半部：DB05 官網備項（編輯性內容）
從 DB05「登記表單」撈取，篩選條件：`文案細項=官網內容`，再用「官網備項」欄位對應到各頁面：
| 頁面 | 路由 | DB05 官網備項值 |
|------|------|----------------|
...
- DB05 表單類型 select 三個選項：報名登記 / 共識互動 / **文化內容**（2026/05/06 改名，舊名「圖文影音」）
...
- 行政區域（select；DB08）
- 單價（number；DB04 活動）
- 庫存異動規則：DB05 表單類型=報名登記 + 登記選項=紀錄庫存 + 庫存細項=進/出/盤 → DB06（明細類型=庫存紀錄）→ DB07
...
- 紀錄（打卡/日誌/請假/加班）、費用（請款/請購）不需「報名登記」上游：表單類型留空，直接用紀錄備項 / 請款請購 區分
