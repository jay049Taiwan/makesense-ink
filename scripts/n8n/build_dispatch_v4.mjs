// 嗨嗨派發中心 v1 (dispatcher v4 — PATCH 外掛狀態=完成)
// 取消內部 PATCH（輪詢 WF 已在 claim 階段設完成）。但保留一鐘變成 idempotent PATCH
// 用法：N8N_API_KEY=xxx node scripts/n8n/build_dispatch_v4.mjs
...
const body = {
  ...
  nodes: [
    ...
    { id: 'd3', name: '呼叫對應 baton', type: 'n8n-nodes-base.httpRequest', typeVersion: 4, position: [800, 300],
      ...
      continueOnFail: true },
    { id: 'd4', name: 'PATCH 外掛狀態=完成', type: 'n8n-nodes-base.httpRequest', typeVersion: 4, position: [1080, 300],
      parameters: {
        method: 'PATCH',
