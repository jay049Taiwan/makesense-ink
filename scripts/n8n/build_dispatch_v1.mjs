// 嗨嗨派發中心 v1 (DB06 dispatch)
// 接 Notion Automation webhook → 解析 智動模式 → 轉打對應 baton webhook → PATCH 外掛狀態=完成
// 用法：N8N_API_KEY=xxx node scripts/n8n/build_dispatch_v1.mjs
...
// === Step 2: 抓警報 WF id 供 errorWorkflow 用 ===
async function findOneId(name) {
  const r = await fetch(N8N + '/workflows?limit=250', { headers: H });
  const j = await r.json();
  return (j.data || []).find(w => w.name === name)?.id;
}
const ALERT_ID = await findOneId('嗨嗨警報 v1');

// === Step 3: 建 dispatch WF ===
const parseCode = `
const MAP = { '搜查': 'search', '分析': 'analyze', '企劃': 'plan', '文案': 'copy', '檢核': 'check' };
const root = $input.first().json || {};
// Notion automation payload 包在 body 或 直接是 payload，視 n8n webhook 設定
const body = root.body || root;
const data = body.data || body;
const props = data.properties || {};
const mode = props['智動模式']?.select?.name || '';
const pageId = data.id || data.page?.id || '';
if (!pageId) throw new Error('缺 page_id（webhook payload 沒有 data.id）');
const modeKey = MAP[mode];
if (!modeKey) throw new Error('未知 智動模式：' + JSON.stringify(mode) + '。應為 搜查/分析/企劃/文案/檢核');
...
const body = {
  ...
  nodes: [
    {
      id: 'd1', name: 'Webhook', type: 'n8n-nodes-base.webhook',
    ...
    {
      id: 'd2', name: '解析模式', type: 'n8n-nodes-base.code',
      typeVersion: 2, position: [520, 300],
      parameters: { language: 'javaScript', jsCode: parseCode },
    },
    {
      id: 'd3', name: '呼叫對應 baton', type: 'n8n-nodes-base.httpRequest',
    ...
    {
      id: 'd4', name: 'PATCH 外掛狀態=完成', type: 'n8n-nodes-base.httpRequest',
  ...
  settings: {
    ...
    ...(ALERT_ID ? { errorWorkflow: ALERT_ID } : {}),
  },
};

const cr = await fetch(N8N + '/workflows', { method: 'POST', headers: H, body: JSON.stringify(body) });
if (!cr.ok) { console.error('建 dispatch 失敗:', cr.status, await cr.text()); process.exit(1); }
...
console.log(`\n========================================`);
console.log(`Webhook URL: https://makesense.zeabur.app/webhook/db06-dispatch`);
console.log(`errorWorkflow: ${ALERT_ID || '(警報 WF 未找到，未設定)'}`);
console.log(`\nNotion Automation 設定指引：`);
console.log(`  DB：DB06 清單明細`);
console.log(`  Trigger：When 外掛狀態 → 執行中`);
console.log(`  Action：Send webhook → POST https://makesense.zeabur.app/webhook/db06-dispatch`);
